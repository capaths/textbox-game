import os
import sys
import subprocess
from shutil import get_terminal_size
from colorama import init
from termcolor import colored

init()

class Console:
  def __init__(self):
    self.update_res()
    self.pos = (1, 1)
    self.clear()

  def get_res(self):
    self.update_res()
    return self._res

  def set_pos(self, x_pos, y_pos):
    self.last_pos = self.pos
    self.pos = (x_pos, y_pos)

  def update_res(self):
    self._res = get_terminal_size()
  
  def write(self, msg, color=None, on_color=None, limit_x=True):
    msg = str(msg)
    sys.stdout.write(colored(msg, color=color, on_color=on_color))
    self.set_pos(self.pos[0] + len(msg), self.pos[1])
    # TODO: Limit horizontal write
    width, height = self.get_res()

  
  def _esc_write(self, msg):
    sys.stdout.write("\x1b[%s" % (msg))

  def clear(self):
    self._esc_write('2J')
    self.set_pos(1, 1)
  
  def read(self):
    x = input()
    self.move(self.pos[0], self.pos[1])
    return x

  def move(self, x, y):
    self._esc_write('%d;%dH' % (y, x))
    self.set_pos(x, y)

  def rel_move(self, x, y):
    if x > 0:
      self._esc_write('%dC' % (x))
    elif x < 0:
      self._esc_write('%dD' % (-x))
    if y > 0:
      self._esc_write('%dA' % (y))
    elif y < 0:
      self._esc_write('%dB' % (-y))
    self.set_pos(self.pos[0] + x, self.pos[1] + y)

  def go_back(self):
    self.set_pos(*self.last_pos)

  def hprint(self, msg, rep=1, step=1):
    for _ in range(rep):
      self.write(msg)
      self.rel_move(step-1, 0)
  
  def vprint(self, msg, rep=1, step=1):
    for _ in range(rep):
      self.write(msg)
      self.go_back()
      self.rel_move(0, step)
